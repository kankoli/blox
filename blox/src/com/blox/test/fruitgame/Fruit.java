...
public class Fruit extends BloxSprite {
	...
	public Fruit(SpriteBatch spriteBatch, float posX, float posY) {
		super(spriteBatch, new Texture(Gdx.files.internal("watermelon.png")),
				30, 30);
		BloxAnimation watermelonAnimation = AnimationBuilder
				...
				.withFrameDuration(Animations.WatermelonFrameDuration)
				.setAsLooping().build();
		...
		BloxAnimation crashAnimation = AnimationBuilder
				...
				.withFrameDuration(Animations.CrashFrameDuration)
				.withFinishListeners(this).build();
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		if (movable) {
			...
			this.velocity = new Vector2(pullStart.x - pullEnd.x,
					-(pullStart.y - pullEnd.y));
			this.velocity.mul(3);
